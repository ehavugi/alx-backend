...
def get_timezone():
    ...
    if g.user:
        user_id = request.args.get('login_as', None)
        try:
            option2 = users.get(int(user_id), None)['timezone']
            if option2:
                pytz.timezone(option2)
                return option2
        except pytz.exceptions.UnknownTimeZoneError:
            pass
    return "UTC"
...
def main():
    ...
    t = utc_time.astimezone(pytz.timezone(get_timezone()))
    t = t.strftime('%b %d %Y %I:%M:%S %p')
    return render_template("index.html", username=g.user, current_time=t)
